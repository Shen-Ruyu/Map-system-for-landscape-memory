...
<%
    try {
        ...
        String sql_update = "select * from yangtzeriverscenery.poetry where title='" +title+ "'";
        //获取你要更新数据的id的数据库信息
...
<form action="poem_doupdate.jsp?title=<%=rs.getString("title")%>" method="post">
	<fieldset>
		<legend>史料信息修改</legend>
		<label for="">
			名称: <input type="text" value="<%=rs.getString("title")%>" name="title">
		</label>
		<label for="">
			作者: <input type="text" value="<%=rs.getString("charac_name") %>" name="charac_name">
		</label>
		<label for="">
			朝代: <input type="text" value="<%=rs.getString("dynasty") %>" name="dynasty">
		</label>
		<label for="">
			内容: <input type="text" value="<%=rs.getString("content") %>" name="content">
		</label>
		<label for="">
			类型: <input type="text" value="<%=rs.getString("poetry_type") %>" name="poetry_type">
		</label>
