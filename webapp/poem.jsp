...
<div class="search d1">
<h6 align="center">史料信息管理</h6>
	  <form name="form" method="post" action="poem_show.jsp">
	  <input type="text" name="name" placeholder="请输入作品名称...">
...
try{
    	 ...
    	 String sql="select * from yangtzeriverscenery.poetry";
    	 ResultSet rs=stmt.executeQuery(sql);
    	 while(rs.next()){
   %>
   <tr>
      <td><%=rs.getString("title") %></td>
      <td><%=rs.getString("charac_name") %></td>
      <td><%=rs.getString("dynasty") %></td>
      <td><%=rs.getString("content") %></td>
      <td><%=rs.getString("poetry_type") %></td>
      <td style="text-align: center;"><a class="btn btn-default btn-sm" href="poem_update.jsp?title=<%=rs.getString("title")%>">修改</a>&nbsp;<a
                           class="btn btn-default btn-sm" href="poem_delete.jsp?title=<%=rs.getString("title")%>">删除</a></td>
   </tr>
   <%
    	 }
     }catch (Exception ex){
